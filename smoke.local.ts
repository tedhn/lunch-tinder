...
await assert.rejects(
  () => caller.room.swipe({ code, userId: crypto.randomUUID(), restaurantId: shared.id, like: true }),
  /Join the room/,
);

// Cards outside the deck are refused.
await assert.rejects(
  () => caller.room.swipe({ code, userId: alice, restaurantId: "not-a-real-place", like: true }),
  /not in this room/,
);

// Alice swipes the whole deck: yes on the first card only.
for (const [i, place] of deck.entries()) {
  await caller.room.swipe({ code, userId: alice, restaurantId: place.id, like: i === 0 });
}

// A double-tap must not inflate anything.
await caller.room.swipe({ code, userId: alice, restaurantId: shared.id, like: true });

state = await caller.room.state({ code });
const aliceView = state.members.find((m) => m.userId === alice)!;
assert.equal(aliceView.swipedCount, deck.length, "duplicate swipe inflated the count");
assert.ok(aliceView.done);
assert.equal(state.phase, "swiping", "one member finishing must not end the round");
assert.equal(state.results, null, "votes leaked before results");

// Bob finishes: yes on the first two cards.
for (const [i, place] of deck.entries()) {
  await caller.room.swipe({ code, userId: bob, restaurantId: place.id, like: i < 2 });
}
...
assert.equal(ranked[0]!.place.id, shared.id, "unanimous pick should rank first");
...
await caller.room.swipe({ code, userId: alice, restaurantId: after[0]!, like: true });
await caller.room.reveal({ code, userId: alice });
state = await caller.room.state({ code });
assert.equal(state.phase, "results");
assert.equal(state.results!.memberCount, 1, "a lurker must not block a unanimous pick");
